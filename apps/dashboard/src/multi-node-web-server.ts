...
const STATIC_FILES = new Map<string, { root: string; name: string }>([
  ["/", { root: LEGACY_PUBLIC_ROOT, name: "index.html" }],
  ["/index.html", { root: LEGACY_PUBLIC_ROOT, name: "index.html" }],
  ["/styles.css", { root: LEGACY_PUBLIC_ROOT, name: "styles.css" }],
  ["/d3.js", { root: LEGACY_PUBLIC_ROOT, name: "d3.js" }],
  ["/scene-view.js", { root: LEGACY_PUBLIC_ROOT, name: "scene-view.js" }],
  ["/timeline.js", { root: LEGACY_PUBLIC_ROOT, name: "timeline.js" }],
  ["/boot.js", { root: APP_PUBLIC_ROOT, name: "boot.js" }],
...
export class MultiNodeDashboardServer {
    ...
    if (this.options.recorder?.status().active) {
      if (this.options.recorder.shouldAutoStop()) {
        void this.options.recorder
          .stop(true)
          .then((status) => this.broadcast("recording", status));
      } else {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/api/history") {
        const seconds = Math.min(
          600,
          Math.max(1, Number(url.searchParams.get("seconds") ?? 120)),
        );
      ...
      if (
        request.method === "POST" &&
        url.pathname === "/api/recording/start"
      ) {
        if (!this.state.readiness.readyForCapture)
          throw new Error(
            `capture blocked: ${this.state.readiness.reasons.join("; ")}`,
          );
      ...
      if (request.method === "POST" && url.pathname === "/api/control")
        throw new Error(
          "device controls are not available in four-node dashboard mode",
        );
  ...
  private requireRecorder(): DashboardRecorder {
    if (!this.options.recorder)
      throw new Error("dashboard recorder is not configured");
  ...
  private async sendStatic(
    ...
    response.writeHead(200, {
      ...
      "Cache-Control": name.endsWith(".html")
        ? "no-store"
        : "public, max-age=300",
    });
