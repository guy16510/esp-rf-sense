...
function renderSignal(raw) {
  ...
  get('streamDot').className = state === 'clear' || state === 'active' ? 'live' : state === 'stale' ? 'stale' : '';
  ...
  get('qualityDetail').textContent = stale ? 'CSI stream stopped updating' : `${number(raw.invalidDatagrams)} invalid datagrams`;
...
function renderDevice(device) {
  ...
  get('firmwareValue').textContent = connected ? `Firmware ${text(firmware.version, 'unknown')}` : text(device?.error, 'No telemetry');
  ...
  get('rssiValue').textContent = health.currentRssi === undefined ? '—' : `${health.currentRssi} dBm`;
  ...
  get('dropValue').textContent = number(health.csiQueueDrops + health.networkQueueDrops).toLocaleString();
  ...
  get('collectorValue').textContent = config.collectorHost ? `${config.collectorHost}:${config.collectorPort}` : '—';
  get('pingValue').textContent = `${number(health.pingReplies).toLocaleString()} / ${number(health.pingRequests).toLocaleString()}`;
  ...
  document.querySelectorAll('[data-control]').forEach((button) => { button.disabled = !connected; });
  updateDiagnosis();
}
...
function renderMemory(valueId, meterId, freeRaw, totalRaw) {
  ...
  get(valueId).textContent = Number.isFinite(total) && total > 0 ? `${bytes(free)} free / ${bytes(total)}` : `${bytes(free)} free`;
...
function cpuText(health) {
  if (Number.isFinite(Number(health.cpuUsagePct))) return `${number(health.cpuUsagePct).toFixed(1)}%`;
  if (Number.isFinite(Number(health.cpuFrequencyMhz))) return `${number(health.cpuFrequencyMhz)} MHz`;
  return 'Unavailable';
}

function updateDiagnosis() {
  const banner = get('diagnosticBanner');
  if (!latestDevice?.connected) return setDiagnosis(banner, 'bad', 'Device API is offline. Check the device IP, Wi-Fi, and npm run rf -- --device http://<device-ip>.');
  const status = latestDevice.status || {};
  const health = latestDevice.health || {};
  if (!status.capture?.active) return setDiagnosis(banner, 'warn', 'Device is online, but capture is stopped. Start capture from the device controls.');
  if (number(health.csiFramesCaptured) === 0) return setDiagnosis(banner, 'bad', 'Capture is active, but the ESP32 has captured no CSI frames. Verify 2.4 GHz Wi-Fi and controlled ping replies.');
  if (number(health.networkBatchesSent) === 0) return setDiagnosis(banner, 'bad', 'CSI is captured, but no UDP batches are being sent. Check the device collector configuration.');
  if (!latestSignal || number(latestSignal.datagrams) === 0) return setDiagnosis(banner, 'bad', 'The ESP32 is sending UDP, but this dashboard receives nothing. Check collector IP, UDP port 5566, and the computer firewall.');
  if (Number.isFinite(latestSignal.ageSec) && latestSignal.ageSec > 3) return setDiagnosis(banner, 'warn', 'The CSI stream is stale. The device may have rebooted or Wi-Fi may have dropped.');
  setDiagnosis(banner, 'good', 'Device telemetry and live CSI are flowing normally. Walk across the router-to-ESP32 path to test disturbance detection.');
}
...
function renderProfile(values) {
  ...
  svg.selectAll('path.profile').data([values]).join('path').attr('class', 'profile').attr('d', line);
}
...
async function control(action, button) {
  ...
  try {
    ...
    setTimeout(() => buttons.forEach((item) => { item.disabled = !latestDevice?.connected; }), 800);
  }
}
...
async function boot() {
  ...
  get('logStatus').textContent = logs.deviceUrl ? 'Connected' : 'Start with npm run rf -- --device URL';
