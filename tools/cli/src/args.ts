// Tiny zero-dependency argv parser shared by the CLI entry points.
...
export function flagStr(flags: Map<string, string>, key: string, env?: string): string | undefined {
  return flags.get(key) ?? (env ? process.env[env] : undefined);
}
